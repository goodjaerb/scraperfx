...
public class GamesDBGame {
    ...
    public List<Image> getImages() {
        ...
        if(images.logo != null) {
            imglist.add(new Image("logo", GamesDbLegacySource.SOURCE_NAME, images.logo.width, images.logo.height, GamesDbLegacySource.IMAGE_BASE_URL + images.logo.path, true));
        }
        if(images.boxarts != null) {
            images.boxarts.stream().forEach((boxart) -> {
                imglist.add(new Image("box-" + boxart.side, GamesDbLegacySource.SOURCE_NAME, boxart.width, boxart.height, GamesDbLegacySource.IMAGE_BASE_URL + boxart.path, true));
            });
        }
        if(images.fanarts != null) {
            ...
            for(final GamesDBFanArt fanart : images.fanarts) {
                i++;
                imglist.add(new Image("fanart", GamesDbLegacySource.SOURCE_NAME, fanart.data.width, fanart.data.height, GamesDbLegacySource.IMAGE_BASE_URL + fanart.data.path, (i == 1)));
            }
        }
        if(images.screenshots != null) {
            ...
            for(final GamesDBScreenshot screenshot : images.screenshots) {
                i++;
                imglist.add(new Image("screenshot", GamesDbLegacySource.SOURCE_NAME, screenshot.data.width, screenshot.data.height, GamesDbLegacySource.IMAGE_BASE_URL + screenshot.data.path, (i == 1)));
            }
        }
        return imglist;
    }
